...
For more advanced usage see `--arg`, `--slurp`, `--raw-file`, `-n` etc and examples at the end of the documentation.
...
`*band*`, `*bor*`, `*bxor*`, `*bsl*`, `*bsr*`, `*bnot*`::
  Bitwise operations as functions. Works the same as jq's math functions. Functions that take one argument use
...
`*delta*`, `*delta_by(f)*`::
  Array with difference between consecutive. `delta` is the same as `delta_by(.b - .a)`.
...
- `da` is `display({array_truncate: 0, string_truncate: 0})` don't truncate array and strings.
- `dd` is `display({array_truncate: 0, string_truncate: 0, display_bytes: 0})` don't truncate array and strings, show all raw bytes.
- `dv` is `display({array_truncate: 0, string_truncate: 0, verbose: true})` don't truncate array and strings and display verbosely.
- `ddv` is `display({array_truncate: 0, string_truncate: 0, display_bytes: 0, verbose: true})` don't truncate array and strings, show all raw bytes and display verbosely.
...
Encoding is done using the `to_xml` function and it will figure what variant that is used based on the input value. It has two optional options `indent` and `attribute_prefix`.
