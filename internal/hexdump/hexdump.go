...
func (d *Dumper) flush() error {
	if _, err := d.cw.Columns[0].Write([]byte(num.PadFormatInt(d.n/int64(d.lineBytes), 16, d.addrLen))); err != nil {
		return err
	}
...
func (d *Dumper) Write(p []byte) (n int, err error) {
	for len(p) > 0 {
		cl := d.n % int64(d.lineBytes)
		if cl == 0 {
			cl = int64(d.lineBytes)
		}
		if cl > int64(len(p)) {
			cl = int64(len(p))
		}
