...
// skip to keep code similar upstream
//
//nolint:errorlint
package csvex
...
func validDelim(r rune, q rune) bool {
	return r != 0 && r != q && r != '\r' && r != '\n' && utf8.ValidRune(r) && r != utf8.RuneError
}
...
func (r *Reader) readRecord(dst []string) ([]string, error) {
	if r.Comma == r.Comment || !validDelim(r.Comma, r.Quote) || (r.Comment != 0 && !validDelim(r.Comment, r.Quote)) {
	...
	quoteBytes := []byte(string(r.Quote))
...
parseField:
	for {
		...
		if len(line) == 0 || !bytes.HasPrefix(line, quoteBytes) {
			...
			for {
				...
				if i >= 0 {
					...
					case rn == r.Quote:
						...
						r.recordBuffer = append(r.recordBuffer, quoteBytes...)
						line = line[quoteLen:]
						pos.col += quoteLen
					case rn == r.Comma:
